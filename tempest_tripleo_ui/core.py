...
class SeleniumElement(object):
    ...
    @classmethod
    def by_element(cls, elem, elem_browser):
        new_elem = SeleniumElement(webelement=elem, browser=elem_browser)
        return new_elem
    ...
    def wait_for_staleness(self, timeout_in_seconds=10):
        ...
        try:
            return ui.WebDriverWait(self.get_browser().get_driver(),
                                    timeout_in_seconds).until(
    ...
    def click_at_offset(self, webelement, x, y):
        actions = webdriver.ActionChains(self.get_browser().get_driver())
        ...
        if self.is_visible() and target.is_visible():
            ...
            if elem_from and elem_to:
                webdriver.ActionChains(
                    self.get_browser().get_driver().drag_and_drop(
